...
# Automatically select CUDA, MPS (Apple GPU), or CPU
device = torch.device(
...
class ImageDataset(torch.utils.data.Dataset):
    def __init__(self, template_dir_path, image_name, thresh_csv=None, transform=None):
        ...
        if self.transform:
            self.image = self.transform(self.image_raw).unsqueeze(0).to(device)
...
image_path = '/Users/saniya.kolangde/Desktop/BluePrint/QATM/QATM_pytorch/sample/qatm_templates/page1_crop_1_zoom2.0.png'
...
#         cv2.imwrite(save_name, d_img)
#     return d_img
template_names = [str(p) for p in dataset.template_path]
def plot_result_multi(image_raw, boxes, indices, template_names, show=False, save_name=None, color_list=None):
    d_img = image_raw.copy()
    if color_list is None:
        color_list = color_palette("hls", len(template_names))
        color_list = list(map(lambda x: (int(x[0]*255), int(x[1]*255), int(x[2]*255)), color_list))
    
    color_map = {}  # map from template_name → color
    for idx, t_name in enumerate(template_names):
        color_map[t_name] = color_list[idx]

    # draw detections
    for i in range(len(indices)):
        t_name = template_names[indices[i]]
        d_img = plot_result(d_img, boxes[i][None, :,:].copy(), color=color_map[t_name])
...
def run_one_sample(model, template, image, image_name):
    ...
    if val.is_cuda or val.device.type == "mps":
        val = val.cpu()   # ✅ Move to CPU
    val = val.numpy()     # ✅ Now safe
    val = np.log(val)
    ...
    for i in range(batch_size):
        ...
        score = np.exp(score / (h*w)) # reverse number range back
        scores.append(score)
...
model = CreateModel(model=models.vgg19(pretrained=True).features, alpha=25, use_cuda=(device.type=="cuda" or device.type=="mps"))
...
d_img, color_map = plot_result_multi(
    dataset.image_raw, boxes, indices, template_names, 
    show=True, save_name='result_sample.png'
)

print("Color mapping (class → BGR):")
...
d_img, color_map = plot_result_multi(
    dataset.image_raw, boxes, indices, template_names,
    show=False, save_name='result_sample.png'
)

# Convert BGR (OpenCV) → RGB for plotting
...
patches = [
    mpatches.Patch(color=np.array(c[::-1])/255.0, label=name)  # reverse BGR → RGB
